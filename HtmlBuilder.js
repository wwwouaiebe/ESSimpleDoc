...
class HtmlBuilder {
	...
	#buildParams ( methodOrProperty ) {
		...
		methodOrProperty.params.forEach (
			param => {
				const paramDoc = methodOrProperty?.doc?.params.find ( a => a.name === param );
	...
	#buildMethodsAndProperties ( methodsOrPropertiesDoc, heading) {
		...
		methodsOrPropertiesDoc.forEach ( 
			methodOrProperty => {
				...
				const getSetPrefix = 'set'=== methodOrProperty.kind || 'get'=== methodOrProperty.kind ? methodOrProperty.kind + ' ' : '';
				const type = methodOrProperty?.doc?.type ?? '';
				this.#html += `<div class="${className}">`; 
				this.#html += `<h3>${asyncPrefix}${staticPrefix}${getSetPrefix}${namePrefix}${methodOrProperty.name} ${type}</h3>`;
				...
				if ( methodOrProperty.doc && ( '' !== methodOrProperty.doc.returns.type || '' !== methodOrProperty.doc.returns.desc ) ) {
					this.#html += `<h4>Returns</h4><div>${methodOrProperty.doc.returns.desc}</div><div>Type : ${methodOrProperty.doc.returns.type}</div>`
				}
	...
	#buildClass ( classDoc ) {
		this.#html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><link type="text/css" rel="stylesheet" href="myDoc.css"></head><body>';
		...
		this.#buildMethodsAndProperties ( 
			...
			).sort ( (a,b) => {return ( a.name +a.kind ).localeCompare ( b.name + b.kind ); } ),
		...
		fs.writeFileSync( classDoc.name + '.html', this.#html );
	}

	build ( doc, sourceFileName ) {
		doc.forEach ( 
			classDoc => { 
